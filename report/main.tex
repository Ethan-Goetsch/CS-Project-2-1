...
% 4. Experiments
\section{Experiments}
The goal of our experiments is to assess the efficiency of generative AI in training our agents, and to compare that to normal AI. To do this, we measured the performance of our generative AI based on (1) technical proficiency, and (2) immersion. Technical proficiency is defined here as how many points our agent can receive from the reward structure in a given number of training iteration. Its formula is $$\sum\limits_{i\in I}k_{i}w_{i},$$ where $I$ is the set of all rewards and punishments, $k$ is the number of times each reward/punishment is given to the agent, and $w$ is the amount of points a certain reward/punishment is worth. $w$ is defined on a normalized scale of $[-1,1]$ where training parameters with a positive value are rewards, and parameters with a negative value are punishments. This is recommended practice in the ML-Agents documentation \cite{unity_ml_agents_documentation}.

Immersion is defined as how natural-seeming the behavior of an agent here. This is primarily measured by intuition, as the goal is to see how believable the behavior of an agent is from the subjective experience of a player.
